  ...
  if (data && data.length > 0) {
    const podiumData = data.map((item, index) => {
      ...
      if (podiumColors === true) {
        if (index == 0) {
          item.placingColor = "#FFD700";
        } //#ddd795
        if (index == 1) {
          item.placingColor = "#d8d8d8";
        } //#c5c4bb
        if (index == 2) {
          item.placingColor = "#cd7f32";
        } //#c09e7d
      }
      return item;
    });
    ...
    podiumData.forEach((item, index) => {
      PodiumDisplay.push(
        <PodiumPlace
          ...
          captionColor={
            captionColor && captionColor != "" ? captionColor : null
          }
          subCaptionColor={
            subCaptionColor && subCaptionColor != "" ? subCaptionColor : null
          }
