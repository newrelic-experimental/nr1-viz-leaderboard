...
const PlacingIndicator = ({ placing, placingColor }: AttributesListProps) => {
  ...
  if (hideRosette !== true && placing > 0 && placing < 4) {
    placingIcon = (
      <div className="placingIndicatorRosette">
    ...
    placingIcon = (
      <div
        className="placingIndicator"
        style={{
          backgroundColor: "#fff",
          boxShadow: `0 0 10px 5px ${placingColor}99`,
        }}
      >
        <span className="placingNumber">{placing}</span>
        <span className="placingSuffix">
          <sup>{createSuffix(placing)}</sup>
        </span>
      </div>
    );
  }

  return <div style={{ paddingTop: "1em" }}>{placingIcon}</div>;
};
